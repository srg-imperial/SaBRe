...
uint64_t decode_j16_offset(uint16_t inst) {
    return decode_jal16_offset(inst);
}
...
inline bool is_jr16(uint16_t inst) { // 16 bit inst. c.jr
    return has_C_extension && ((inst & 0x3) == 0b10)
	&& ((inst & 0x7f) == 0b10)
	&& (((inst & 0xf000) >> 12) == 0b1000)
	&& ((inst & 0xf80) != 0);
}
...
inline bool is_control_flow_inst(uint32_t inst) {
    return is_jal32(inst) || is_branch32(inst) 
        || is_jal16(inst) || is_j16(inst)
        || is_beqz16(inst) || is_bnez16(inst)
	|| is_jr16(inst) || is_jalr16(inst);
}
...
uint64_t inst_offset64(uint32_t inst) {
	...
	fprintf(stderr, "inst_offset64 fails: attempt to extract offset from not control flow instruction\n");
	assert(false);
    }
    return offset;
}
...
inline bool is_safe_insn(uint32_t insn) {
  return insn && (
	  ...
	  || (has_C_extension && (
		  ...
		  || ((insn & 0x3) == 0x1 && (insn & 0xe000) == 0x6000) // c.addiw, c.li
