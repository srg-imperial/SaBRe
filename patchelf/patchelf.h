...
#define ElfFileParamNames                                                      \
  Elf_Ehdr, Elf_Phdr, Elf_Shdr, Elf_Addr, Elf_Off, Elf_Dyn, Elf_Sym,           \
      Elf_Verneed, Elf_Versym
...
private:
  struct CompPhdr {
    ...
    bool operator()(const Elf_Phdr &x, const Elf_Phdr &y) {
      ...
      if (elfFile->rdi(y.p_type) == PT_PHDR)
        return false;
      if (elfFile->rdi(x.p_type) == PT_PHDR)
        return true;
  ...
  tryFindSectionHeader(const SectionName &sectionName);
