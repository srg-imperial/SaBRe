...
ElfFile<ElfFileParamNames>::ElfFile(FileContents fContents)
  ...
  if (rdi(hdr()->e_phoff) + rdi(hdr()->e_phnum) * rdi(hdr()->e_phentsize) >
      fileContents->size())
  ...
  if (rdi(hdr()->e_shoff) + rdi(hdr()->e_shnum) * rdi(hdr()->e_shentsize) >
      fileContents->size())
  ...
  char *shstrtab =
      (char *)fileContents->data() + rdi(shdrs[shstrtabIndex].sh_offset);
...
template <ElfFileParams>
Elf_Shdr &
ElfFile<ElfFileParamNames>::findSectionHeader(const SectionName &sectionName) {
...
template <ElfFileParams>
std::optional<std::reference_wrapper<Elf_Shdr>>
...
template <ElfFileParams>
unsigned int
ElfFile<ElfFileParamNames>::getSectionIndex(const SectionName &sectionName) {
  for (unsigned int i = 1; i < rdi(hdr()->e_shnum); ++i)
...
ElfFile<ElfFileParamNames>::replaceSection(const SectionName &sectionName,
  ...
  if (i != replacedSections.end()) {
    ...
    s = std::string((char *)fileContents->data() + rdi(shdr.sh_offset),
                    rdi(shdr.sh_size));
  }
...
void ElfFile<ElfFileParamNames>::writeReplacedSections(Elf_Off &curOff,
  ...
  for (auto &i : replacedSections) {
    ...
    if (rdi(shdr.sh_type) != SHT_NOBITS)
      memset(fileContents->data() + rdi(shdr.sh_offset), 'X',
             rdi(shdr.sh_size));
  }
...
void ElfFile<ElfFileParamNames>::rewriteSectionsLibrary() {
  ...
  for (auto &phdr : phdrs) {
    Elf_Addr thisPage =
        roundUp(rdi(phdr.p_vaddr) + rdi(phdr.p_memsz), getPageSize());
...
std::string ElfFile<ElfFileParamNames>::getInterpreter() {
  ...
  return std::string((char *)fileContents->data() + rdi(shdr.sh_offset),
                     rdi(shdr.sh_size) - 1);
}
...
void ElfFile<ElfFileParamNames>::printNeededLibs() // const
{
  ...
  const Elf_Dyn *dyn =
      (Elf_Dyn *)(fileContents->data() + rdi(shdrDynamic.sh_offset));
